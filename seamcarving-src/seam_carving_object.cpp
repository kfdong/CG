...
real_t alpha = 0.5;
const int MaxN = 6000, CHANNEL = 3;
const real_t INF = 1e12;
int image[MaxN][MaxN][CHANNEL];
bool protc[MaxN][MaxN], remv[MaxN][MaxN];
bool flag[MaxN][MaxN];
...
void read_mark(char *file, bool mark[MaxN][MaxN]) {
	int width, height, bpp;
    uint8_t* rgb_image = stbi_load(file, &width, &height, &bpp, 3);
	...
	for (int i = 0; i < height; ++i)
		for (int j = 0; j < width; ++j) {
			int sum = 0;
			for (int k = 0; k < 3; ++k)
				sum = sum + rgb_image[(i * width + j) * CHANNEL + k];
			if (sum >= 255 * 2.99) mark[i][j] = 0;
			else mark[i][j] = 1;
		}
...
void compute_energy(real_t alpha) {
	for (int i = 0; i < height; ++i)
		for (int j = 0; j < width; ++j) {
			...
			if (remv[x][y]) energy[i][j] += -INF;
			if (protc[x][y]) energy[i][j] += INF * 100;
		}
}
...
void find_column_seam(int width, int height, vector<vector<int> > &paths) {
	...
	/* find path */
	static pair <real_t, int> pt[MaxN];
	for (int j = 0; j < width; ++j) 
	...
	for (int j = 0; j < width; ++j) {
		...
		if ((int)p.size() == height) {
			...
			for (int i = 0; i < height; ++i) 
				flag[i][p[i]] = 1;
			paths.push_back(p);
			if (current_cost > min_cost * 1.5) break;
		}
	}
}
...
int main(int argc, char **argv) {
	if (argc <= 1) {
		printf("usage %s <filename> alpha [--protect <filename>] [--remove <filename>]\n", argv[0]);
		return 0;
	}
	alpha = stod(argv[2]);
	for (int i = 3; i + 1 < argc; i += 2)
		if (strcmp(argv[i], "--protect") == 0)
			read_mark(argv[i + 1], protc);
	for (int i = 3; i + 1 < argc; i += 2)
		if (strcmp(argv[i], "--remove") == 0)
			read_mark(argv[i + 1], remv);
	...
	seam_carving(int(0.1 * height), int(0.1 * width));
