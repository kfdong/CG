...
class lambertian_mesh : public material {
public:
	vect col_0, col_1;
	real_t size = 0.3;
	lambertian_mesh(const vect &col_0, const vect &col_1) : col_0(col_0), col_1(col_1) {
		type = LAMBERTIAN;
	}
	bool scatter(const ray &in, const hit_record &h, vect &attenuation, ray &out) const; 
};

bool lambertian_mesh::scatter(const ray &in, const hit_record &h, vect &attenuation, ray &out) const {
	vect v = h.norm + random_in_unit_ball();
	out = ray(h.p, v);
	int pos = (int(floor(h.p.e[0] / size)) + int(floor(h.p.e[1] / size)) + int(floor(h.p.e[2] / size))) % 2;
	if (pos == 0) attenuation = col_0;
	else attenuation = col_1;
	return true;
}
...
public:
	...
	vect vx, vy;
	lambertian_texture(const char *name, const vect &_vx, const vect &_vy) {
		rgb_image = stbi_load(name, &width, &height, &bpp, 3);
		vx = _vx, vy = _vy;
		type = LAMBERTIAN;
	}
	bool scatter(const ray &in, const hit_record &h, vect &attenuation, ray &out) const; 
...
bool lambertian_texture::scatter(const ray &in, const hit_record &h, vect &attenuation, ray &out) const {
	vect v = h.norm + random_in_unit_ball();
	out = ray(h.p, v);
	int x = floor(dot(h.p, vx)), y = floor(dot(h.p, vy));
	x = cmod(x + height / 2, height);
	y = cmod(y + width / 2, width);
	attenuation = vect(rgb_image[(x * width + y) * 3 + 0], rgb_image[(x * width + y) * 3 + 1], rgb_image[(x * width + y) * 3 + 2]) / 255;
	return true;
}
...
public:
	...
	real_t shatter_radius;

	point_light() {}
	point_light(const vect &_x, const vect &_norm, const vect &_color, real_t _r = 1.0) {
		x = _x;
		norm = _norm.norm();
		color = _color;
		shatter_radius = _r;
	...
	ray emit(vect &col_in) {
		col_in = color;
		ray ret(x, norm + random_in_unit_ball() * shatter_radius);
		return ret;
	}
};

thread_local std::normal_distribution<double> gaussian(0, 1);
class gaussian_light: public lights {
public:
	vect x; /* position */
	vect norm; /* outgoing norm of the hemisphere */
	vect color; /* color of the light */
	...
	gaussian_light() {}
	gaussian_light(const vect &_x, const vect &_norm, const vect &_color, real_t _r = 1.0, real_t _sigma = 0.5) {
		x = _x;
		norm = _norm.norm();
		color = _color;
		shatter_radius = _r;
		...
		dx = vect(0, 1, 0);
		dx = (dx - dot(dx, norm) * norm).norm();
		dy = cross(dx, norm);
	}

	ray emit(vect &col_in) {
		col_in = color;
		vect s = x + sigma * (dx * gaussian(gen) + dy * gaussian(gen));
		vect v = norm + random_in_unit_ball() * shatter_radius;
		return ray(s, v);
	}
};
...
public:
	...
	vect axis;
	Bezier *f;
	vector <real_t> iy, ix, it, ix_tangent;
	bounding_box *bd;
	...
	rotation(real_t _dir, vect _axis, Bezier *_f, material *_mat) : dir(_dir), axis(_axis), f(_f) {
		...
		for (int i = 0; i <= BEZIER_SAMPLE; ++i) {
			...
			ix_tangent.push_back(0);
		}
		for (int i = 0; i < BEZIER_SAMPLE; ++i) {
			real_t x, y, dx, dy;
			real_t tmid = (0.5 + i) / BEZIER_SAMPLE;
			f->eval(tmid, x, y);
			f->eval_deriv(tmid, dx, dy);
			ix_tangent[i] = max(ix_tangent[i], x + (iy[i] - y) / dy * dx);
			ix_tangent[i + 1] = max(ix_tangent[i + 1], x + (iy[i + 1] - y) / dy * dx);
		}
	}
	virtual hit_record hit(const ray &r) const;
	void get_init(const ray &r, vector <real_t> &init, const vector <real_t> &ix, const vector <real_t> &iy, const vector <real_t> &it) const;
};

void rotation::get_init(const ray &r, vector <real_t> &init, const vector <real_t> &ix, const vector <real_t> &iy, const vector <real_t> &it) const {
...
hit_record rotation::hit(const ray &r) const {
	hit_record ret;
	static const int MAXP = 15;

	if (bd != NULL && !bd->hit(r)) return ret;
	real_t x, y, dx, dy;
	vector <real_t> init;
	init.reserve(NEWTON_INIT);
	get_init(r, init, ix, iy, it);
	get_init(r, init, ix_tangent, iy, it);
	for (int i = 0; i < NEWTON_INIT; ++i)
		init.push_back((i + 0.5) / NEWTON_INIT);
	/*
	bool flag = 0;
		...
		if (fabs(r.v.e[1]) > EPS) {
			...
			if (delta > 0) {
				...
				if (k >= EPS && (ret.hit == false || (ret.hit == true && ret.t > k))) {
					vect p = r[k];
					vect norm = vect(p.e[0] - axis.e[0], 0, p.e[2] - axis.e[2]).norm();
					norm *= dir;
					if (-dot(norm, r.v) > 0) {
						ret.hit = true;
						ret.t = k;
	...
	if (fabs(r.v.e[1]) <= EPS) {
		...
		for (auto t : init) {
			bool flag = 0;
			for (int sp = 0; sp < NEWTON_LIMIT; ++sp) {
				real_t f = 0, df = 0, pw = 1;
				for (int i = 0; i + 1 < MAXP; ++i, pw = pw * t) {
					f += pw * p[i];
					df += pw * p[i + 1] * (i + 1);
				}
				if (fabs(f) < NEWTON_EPS) { flag = 1; break; } 
			...
			if (flag) {
				...
				for (int i = 0; i < MAXP; ++i) p[i] = tmp[i];
			}
		}
	...
	if (fabs(r.v.e[1]) > EPS) {
		...
		for (auto t : sol) {
			...
			if (k >= EPS && (ret.hit == false || (ret.hit == true && ret.t > k))) {
				vect norm;
				vect p = r[k];
				norm = vect(p.e[0] - axis.e[0], 0, p.e[2] - axis.e[2]).norm();
				norm.e[1] = -dx / dy;
				norm *= dir;
				norm = norm.norm();
				// cerr << k << ' ' << r[k] << ' ' << norm << ' ' << -dot(norm, r.v) << ' ' << ret.hit << ' ' << t << ' ' << dy << ' ' << dx << endl;
				if (-dot(norm, r.v) > EPS) {
					ret.p = p;
					ret.hit = true;
					ret.t = k;
			...
			// cerr << ret.p << ' ' << ret.t << ' ' << ret.norm << ' ' << ret.hit << endl;
			// cerr << ((lambertian*)mat)->albedo << endl;
		}
	}
	return ret;
}
